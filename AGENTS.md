...
- `mcpServers` are bridged into pi via the bundled `src/mcp/pi-mcp-extension.ts` (spawned with `pi -e` + `PI_ACP_MCP_SERVERS` env); the extension registers MCP tools as native pi tools.
